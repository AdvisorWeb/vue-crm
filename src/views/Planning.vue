		...
		<section v-else>
			<div v-for="c in categories" :key="c.id">
				<p>
					...
					{{c.spend | currency('BYN')}} {{ "Of" | localize }} {{c.limit | currency('BYN')}}
				</p>
...
export default {
	...
	async mounted() {
		...
		this.categories = categories.map(cat => {
			...
			const tooltip = `${tooltipValue < 0 ? 'Превышения на' : 'Осталось'} ${currencyFilter(Math.abs(tooltipValue))}`
			return {
				...cat,
				progressPercent,
				progressColor,
